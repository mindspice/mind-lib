package io.mindspice.mindlib.data.geometry;

import java.util.concurrent.locks.StampedLock;


public class IAtomicLine2 implements ILine2 {
    private IMutVector2 start;
    private IMutVector2 end;
    private final StampedLock lock = new StampedLock();

    IAtomicLine2(IVector2 start, IVector2 end) {
        this.start = IVector2.ofMutable(start);
        this.end = IVector2.ofMutable(end);
    }

    IAtomicLine2(int startX, int startY, int endX, int endY) {
        start = IVector2.ofMutable(startX, startY);
        end = IVector2.ofMutable(endX, endY);
    }

    @Override
    public IMutVector2 start() {
        long stamp = lock.tryOptimisticRead();
        IMutVector2 currentStart = start;
        if (!lock.validate(stamp)) {
            stamp = lock.readLock();
            try {
                currentStart = start;
            } finally {
                lock.unlockRead(stamp);
            }
        }
        return currentStart;
    }

    @Override
    public IMutVector2 end() {
        long stamp = lock.tryOptimisticRead();
        IMutVector2 currentEnd = end;
        if (!lock.validate(stamp)) {
            stamp = lock.readLock();
            try {
                currentEnd = end;
            } finally {
                lock.unlockRead(stamp);
            }
        }
        return currentEnd;
    }

    @Override
    public double distance() {
        long stamp = lock.tryOptimisticRead();
        double distance;
        double deltaX = end.x() - start.x();
        double deltaY = end.y() - start.y();
        distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
        if (!lock.validate(stamp)) {
            stamp = lock.readLock();
            try {
                deltaX = end.x() - start.x();
                deltaY = end.y() - start.y();
                distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
            } finally {
                lock.unlockRead(stamp);
            }
        }
        return distance;
    }

    @Override
    public boolean isPointLine() {
        long stamp = lock.tryOptimisticRead();
        boolean pointLine = start.equals(end);
        if (!lock.validate(stamp)) {
            try {
                stamp = lock.readLock();
                pointLine = start.equals(end);
            } finally {
                lock.unlockWrite(stamp);
            }
        }
        return pointLine;
    }

    @Override
    public boolean intersects(ILine2 otherLine) {
        // Find the four orientations needed for general and
        // special cases
        long stamp = lock.readLock();
        try {
            int o1 = orientation(start(), end(), otherLine.start());
            int o2 = orientation(start(), end(), otherLine.end());
            int o3 = orientation(otherLine.start(), otherLine.end(), start());
            int o4 = orientation(otherLine.start(), otherLine.end(), end());

            // General case
            if (o1 != o2 && o3 != o4) { return true; }

            // Special Cases
            // start(), end() and otherLine.start() are collinear and otherLine.start() lies on segment start()end()
            if (o1 == 0 && onSegment(start(), otherLine.start(), end())) { return true; }

            // start(), end() and otherLine.end() are collinear and otherLine.end() lies on segment start()end()
            if (o2 == 0 && onSegment(start(), otherLine.end(), end())) { return true; }

            // otherLine.start(), otherLine.end() and start() are collinear and start() lies on segment otherLine.start()otherLine.end()
            if (o3 == 0 && onSegment(otherLine.start(), start(), otherLine.end())) { return true; }

            // otherLine.start(), otherLine.end() and end() are collinear and end() lies on segment otherLine.start()otherLine.end()
            if (o4 == 0 && onSegment(otherLine.start(), end(), otherLine.end())) { return true; }

            return false; // Doesn't fall in any of the above cases
        } finally {
            lock.unlockRead(stamp);
        }
    }

    private int orientation(IVector2 p, IVector2 q, IVector2 r) {
        long stamp = lock.readLock();
        try {
            int val = (q.y() - p.y()) * (r.x() - q.x()) -
                    (q.x() - p.x()) * (r.y() - q.y());

            if (val == 0) {
                return 0; // collinear
            }

            return (val > 0) ? 1 : 2; // clock or counterclock wise
        } finally {
            lock.unlockRead(stamp);
        }
    }

    private boolean onSegment(IVector2 p, IVector2 q, IVector2 r) {
        return q.x() <= Math.max(p.x(), r.x()) && q.x() >= Math.min(p.x(), r.x()) &&
                q.y() <= Math.max(p.y(), r.y()) && q.y() >= Math.min(p.y(), r.y());
    }

    public void setStart(int x, int y) {
        long stamp = lock.writeLock();
        try {
            start.setXY(x, y);
        } finally {
            lock.unlockWrite(stamp);
        }
    }

    public void setStart(IVector2 start) {
        long stamp = lock.writeLock();
        try {
            this.start.setXY(start.x(), start.y());
        } finally {
            lock.unlockWrite(stamp);
        }
    }

    public void setEnd(int x, int y) {
        long stamp = lock.writeLock();
        try {
            end.setXY(x, y);
        } finally {
            lock.unlockWrite(stamp);
        }
    }

    public void setEnd(IVector2 end) {
        long stamp = lock.writeLock();
        try {
            this.end.setXY(end.x(), end.y());
        } finally {
            lock.unlockWrite(stamp);
        }
    }

    public void shiftLine(IVector2 newEnd) {
        long stamp = lock.writeLock();
        try {
            start.setXY(end);
            end.setXY(newEnd);
        } finally {
            lock.unlockWrite(stamp);
        }
    }

    public void shiftLine(int newX, int newY) {
        long stamp = lock.writeLock();
        try {
            start.setXY(end);
            end.setXY(newX, newY);
        } finally {
            lock.unlockWrite(stamp);
        }
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) { return true; }
        if (!(o instanceof ILine2 iLine2)) { return false; }
        if (!start.equals(iLine2.start())) { return false; }
        return end.equals(iLine2.end());
    }

    @Override
    public int hashCode() {
        long stamp = lock.tryOptimisticRead();
        int result = start != null ? start.hashCode() : 0;
        result = 31 * result + (end != null ? end.hashCode() : 0);
        if (!lock.validate(stamp)) {
            stamp = lock.readLock();
            try {
                result = start != null ? start.hashCode() : 0;
                result = 31 * result + (end != null ? end.hashCode() : 0);
            } finally {
                lock.unlockRead(stamp);
            }
        }
        return result;
    }

    @Override
    public String toString() {
        return "[" + start + ", " + end + "]";
    }
}
